...
logger = logging.getLogger('cog.add')

class AddCog(commands.Cog, name="EconomyAdd"):
    ...
    @app_commands.command(name="add", description="[Admin] Add currency to a user for work done in a specific context.")
    @app_commands.guild_only()
    @app_commands.check(actual_is_not_restricted_check)
    @app_commands.describe(user="The user to add currency to. Defaults to yourself.", amount="The amount of currency to add.", context="The channel or forum post where the task occurred.", task="The task name related to this addition.", reason="The reason for adding currency.")
    @app_commands.autocomplete(context=context_autocomplete, task=task_autocomplete) # <-- USE a
    async def add(self, interaction: Interaction, amount: app_commands.Range[float, 0.01, None], context: str, task: str, reason: str, user: discord.Member = None):
        target_user = user or interaction.user
        if target_user.id != interaction.user.id:
            if not interaction.user.guild_permissions.administrator:
                await interaction.response.send_message("You have permission to use `/add`, but only for yourself. Administrator permissions are required to add money to other users.", ephemeral=True)
                return
        if target_user.bot:
            await interaction.response.send_message("You cannot add currency to a bot!", ephemeral=True)
            return
        try:
        ...
        if isinstance(context_obj, discord.TextChannel):
            ...
            else: error_msg = "Adding funds via Text Channel is disabled. An admin can enable it in `/settings`."
            ...
            else: error_msg = "Adding funds via Forum Post is disabled. An admin can enable it in `/settings`."
        else:
        ...
        if not await database.get_task(guild_id, guild_name, task_upper):
            await interaction.followup.send(f"Task '{task}' not found. Please create it in `/settings`.", ephemeral=True)
            return
        success = await database.add_money(guild_id, guild_name, target_user.id, target_user.name, amount, reason, task_upper, interaction.user.id, interaction.user.name, context_data)
        if success:
            embed = discord.Embed(title=f"{EMOJIS['MONEY']} Money Added", description=f"{EMOJIS['ADD']} Added **{format_currency(amount)}** to {target_user.mention} for work in {context_mention} on **{discord.utils.escape_markdown(task_upper)}**.", color=COLORS["SUCCESS"], timestamp=datetime.now()).add_field(name="Reason", value=discord.utils.escape_markdown(reason))
            if target_user.avatar: embed.set_thumbnail(url=target_user.avatar.url)
            await interaction.followup.send(embed=embed)
        else:
            await interaction.followup.send("Failed to add currency. An error occurred.", ephemeral=True)
